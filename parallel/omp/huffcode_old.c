...
static unsigned char* memory_encode_file(FILE *in, FILE *out, unsigned long sz);
static int memory_decode_file(FILE *in, FILE *out, unsigned long sz);
...
main(int argc, char** argv)
{
	...
	if(memory)
	{
		if (compress) {
			...
			for(i = 0; i < 4; ++i) {
				buf[i] = memory_encode_file(fp[i], out, (unsigned long) (sz / 4));
			}
		}
		else {
			for(i = 0; i < 4; ++i) {
				buf[i] = memory_decode_file(fp[i], out, (unsigned long) (sz / 4));
			}
		}
...
memory_encode_file(FILE *in, FILE *out, unsigned long sz)
{
	unsigned char *buf = NULL, *bufout = NULL;
	unsigned int i, len = 0, cur = 0, inc = 1024, bufoutlen = 0;
	...
	for(i = 0; i < (unsigned int)sz; i += inc)
	{
		...
		buf = tmp;
		if(cur + inc > sz) {
			fread(buf + cur, 1, (unsigned int)(sz - cur), in);
		} 
		else {
...
static int
memory_decode_file(FILE *in, FILE *out, unsigned long sz)
{
	unsigned char *buf = NULL, *bufout = NULL;
	unsigned int i, len = 0, cur = 0, inc = 1024, bufoutlen = 0;
	assert(in && out);
